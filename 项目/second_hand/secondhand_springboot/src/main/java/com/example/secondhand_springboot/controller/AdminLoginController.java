...
public class AdminLoginController {
    ...
    public Result login(@RequestBody LoginRequest loginRequest){
        LoginDTO login=adminLoginService.login(loginRequest);
        return  Result.success(login);
    }

    @GetMapping("/{id}")
    public Result getById(@PathVariable Integer id){
        Login login=adminLoginService.getById(id);
        return Result.success(login);
    }

    @GetMapping("/u_name/{u_name}")
    public Result getByUserName(@PathVariable String u_name){
        Login admin=adminLoginService.getByUserName(u_name);
...
//            String token=TokenUtils.genToken(currentAdmin.getId().toString(),currentAdmin.getPassword(),15);
            String url="http://localhost:9090/api/adminlogin/file/download/"+flag;
    ...
    public Result upAvatar(@RequestBody Login login){
        adminLoginService.upAvatarById(login);
